...
当前 TS-07 已完成现有 Chrome 高保真 Demo 的最小成本候选审计：正常音频 cue、暂停恢复和重复定位达到 `<250ms` 门槛，但运行中音频失败后的视觉降级从头开始，状态保持 `candidate_run_complete` 且候选门禁失败。详见 [`ts-07-unified-playback-timeline/`](ts-07-unified-playback-timeline/)。
