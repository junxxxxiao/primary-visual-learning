...
声音链路：`公开入口 → 发起提问 → 旁白确认问题 → 词义校准 → 旁白引导预测 → 居中引入 → 三段自动连播讲解 → 发送文字或在当前画布模拟语音转写 → 真空关联讲解 → 连接旁白 → 返回原主题 → 可选动手实验 → 音叉迁移 → 必要时木琴迁移 → 结果 → 问卷`。数学链路：`固定题图输入 → 问题确认 → 准备 → 四段自动连播讲解 → 120 米围栏迁移 → 结果`。拍照和语音按钮只展示入口体感，不会申请相机或麦克风权限，也不会上传媒体。
...
- 手机一屏：核心讲解、实验区、三段导航和播放控制在单屏内完整显示，不要求上下滚动；
...
- 字幕开关固定属于底部统一播放器，顶部导航只保留返回、品牌、主题和播放状态；不得由页面脚本把顶部文字转换成字幕控件；
...
浏览器完整验收覆盖固定 `?viewport=phone` 的 `390x844` 审阅画布、真实响应式 `360x800`、`1180x820` 和 `1366x768`，并检查问题确认与预测旁白、讲解三段自动连播、讲解/操作分离、固定关联追问、讲解后慢放操作、子片段恢复、两次迁移、三类结果、问卷两种状态、点击目标、减少动态效果和控制台错误。每次 UI 或交互审阅都必须同时检查 `?viewport=phone`；该参数不得随桌面窗口伸缩。
...
所有讲解动画都遵循系统级[视觉场景布局契约](../docs/adr/0003-visual-scene-layout-contract.md)，完整位于共享灰色讲解画布安全内边界。手机和平板分别构图；`overflow: hidden` 只用于阻止意外绘制污染，不得以裁切替代响应式重排。验收需逐段检查两端的起始、关键过程、最终、暂停、恢复、交互后与减少动态效果状态，同时检查元素/轨迹边界框和 scroll/client 隐藏裁切，并输出机器可读结果。
