...
所有讲解动画都必须完整位于共享灰色讲解画布内。`overflow: hidden` 只用于阻止意外绘制污染，不得以裁切替代响应式重排；验收需逐段检查手机和平板的起始、过程与最终状态边界框。

数学候选分支已通过脚本语法、旁白文件覆盖、WAV 格式、加载文案、放大入口和画布审计静态门禁。当前自动化环境禁止接管 `file://` 页面，因此最终网格修改后的手机/平板逐段浏览器边界框与人工裁切检查仍待完成；合并前必须在 `?viewport=phone` 与 `?viewport=tablet` 逐段确认 `window.__demoAuditCanvasBounds()` 返回空数组。
