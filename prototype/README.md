...
2026-07-22 严格验收还覆盖：校准和预测不可跳过、先轻拨后用力拨、连续两次无效拖动后才显示保底按钮、琴弦跟随拖动、真实音频时长、手机固定播放区和完整滚动空间。当前验收进一步覆盖页面、条件性画布、小知消息区和 Overlay 的滚动归属、边界传递与锁定。2026-07-23 更新后的门禁以核心讲解完成为准；实验和关联追问均为可选体验，不阻塞迁移。
